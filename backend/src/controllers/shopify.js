...
export const oauthCallback = asyncHandler(async (req, res) => {
  ...
  if (decodedState.shop !== shop) {
    return res
      .status(400)
      .json(errorResponse("Shop mismatch between state and callback parameters."));
  }
  ...
  try {
    ...
    const shouldRedirect = redirectBase?.startsWith("https://");
    if (shouldRedirect) {
    ...
    return res
      ...
      .json(
        successResponse(
          { shop, scope },
          "Shopify store connected successfully. You can close this window.",
        ),
      );
  } catch (error) {
...
export const fetchCustomers = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(
      `https://${store.shopDomain}/admin/api/2024-10/customers.json?${searchParams.toString()}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      return res
        ...
        .json(errorResponse("Failed to fetch customers from Shopify."));
    }
    ...
    return res
      .status(200)
      .json(successResponse(enrichedCustomers, "Customers fetched successfully."));
  } catch (error) {
